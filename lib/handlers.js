...
/**
 * Recurse through test suites to create one
...
export function makeMatrix({
  ...
  return {
    // by default export all matrices
    export: true,
    ...suite,
 ...
 *  template.
*/
export function makeTemplateData({reports}) {
  const context = {
    matrices: [],
    tables: [],
    summary: []
  ...
  for(const report of reports) {
    report.tests = addSubTests({tests: report.tests, suites: report.suites});
    if(report.matrix) {
