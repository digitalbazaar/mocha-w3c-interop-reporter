...
function InteropReporter(runner, options = {}) {
  ...
  const {
    ...
    // rename abstract to abstractTemplate to prevent confusion with
    // js keyword abstract
    abstract: abstractTemplate = config.templates.abstract,
    body = config.templates.body,
    matrix = config.templates.matrix,
    table = config.templates.table,
    head = config.templates.head,
    metrics = config.templates.metrics,
    statistics = config.templates.statistics,
